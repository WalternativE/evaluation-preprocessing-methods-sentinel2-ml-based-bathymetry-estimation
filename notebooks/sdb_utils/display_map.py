# This set of utility modules was copied from the opendatacube repository and adapted for personal use
# https://github.com/opendatacube/datacube-notebooks/blob/master/utils/notebooks/display_map.py
import folium
import itertools
import math
import numpy as np
...
def display_bounding_box_for_gpd(gdf: gpd.GeoDataFrame):
    """ Generates a folium map with a lat-lon bounded rectangle drawn on it. Folium maps can be

    Args:
        gdf: (geopandas.GeoDataFrame): a geo dataframe which supplies the total bounds for the aoi bounding box
    ...
    """
    crs = 'EPSG:4326'
    minx, miny, maxx, maxy = gdf.to_crs(crs).total_bounds
    map_lat = miny, maxy
    map_lon = minx, maxx

    return display_map(map_lat, map_lon)
